package HomePractice.GeeksForGeeks.Arrays.Easy;

import java.util.Scanner;

/*
Working with 2D arrays is quite important. Here we will do swapping of column in a 2D array. You are given a matrix M or r rows and c columns. You need to swap the first column with the last column.

Example:

Input:
3 4
1 2 3 4
4 3 2 1
6 7 8 9

Output:
4 2 3 1
1 3 2 4
9 7 8 6
Your Task:
Since this is a function problem, you don't need to take any input. Just complete the provided function interchange(int, int , int ) that take matrix, rows and columns as parameters.

Constraints:
1 <= r,c <= 100
 */
public class MatrixInterchangeJava {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.print("Enter the matrix size of row : ");
        int r = sc.nextInt();
        System.out.print("Enter the matrix size of column: ");
        int c = sc.nextInt();
        System.out.println("Enter the matrix elements : ");
        int m[][] = new int[r][c];
        for(int i=0;i<r;i++)
        {
            for(int j=0;j<c;j++)
            {
                System.out.print("m["+i+"]["+j+"]=");
                m[i][j] = sc.nextInt();
            }
        }
        interchange(m,r,c);
    }

    static void interchange(int a[][], int r, int c) {

        // Your code here
        for(int i=0;i<r;i++)
        {
            int temp = a[i][0];
            a[i][0] =a[i] [c-1];
            a[i][c-1] = temp;
        }

        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
                System.out.print(a[i][j] + " ");
            }
            System.out.println();
        }
    }
}
